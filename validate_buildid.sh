...
# This script checks if the local build ID of a Counter-Strike 2 dedicated server
...
local_buildid=$(grep -oP '"buildid".{1,}"(.*?)"' /home/steam/serverfiles/steamapps/appmanifest_730.acf | sed -n 's/.*"buildid".*"\(.*\)"/\1/p')
